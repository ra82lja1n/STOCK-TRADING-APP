...
const StockChart = () => {
  ...
  params: {
    ...
    try {
      ...
      await axiosInstance.post('/buyStock', {user: userId, symbol: id, name: res.data.data[0].instrument_name, stockType: buyType, stockExchange: stockExchange, price: stockPrice,  count: buyQuantity, totalPrice: stockPrice * buyQuantity}).then(
      (response)=>{
          setBuyQuantity(0);
          setBuyType(0);
          navigate('/history');
  ...
  params: {
    ...
    try {
      const res = await axios.request(options);
      await axiosInstance.post('/sellStock', {user: userId, symbol: id, name: res.data.data[0].instrument_name, stockType: sellType, price: stockPrice,  count: sellQuantity, totalPrice: stockPrice * sellQuantity}).then(
      (response)=>{
          setBuyQuantity(0);
          setBuyType(0);
          navigate('/history');
