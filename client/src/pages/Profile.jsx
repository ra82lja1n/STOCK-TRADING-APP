...
const Profile = () => {
  ...
  const deposit = async (e) => {
    e.preventDefault();
    await axiosInstance
      .post("/deposit", { user: userId, depositAmount, depositMode })
      .then((response) => {
  ...
  const withdraw = async (e) => {
    e.preventDefault();
    await axiosInstance
      .post("/withdraw", { user: userId, withdrawAmount, withdrawMode })
      .then((response) => {
        localStorage.setItem("balance", response.data.balance);
        setUserData({ ...userData, balance: response.data.balance });
        fetchTransactions();
  ...
  const fetchTransactions = async () => {
    await axiosInstance.get("/transactions").then((response) => {
      if (response.data == "Auth Error") setTransactions([]);
      else setTransactions(response?.data?.reverse());
    }).catch((err)=>{
      console.log(err)
    })
  };
      ...
      <div className="profileBox">
        ...
        <div className="profileBox-body">
          ...
          <div className="cash-actions">
            ...
            >
              <RiHistoryLine className="cash-action-icons" /> Transaction
              History
            </button>
          </div>
        </div>
      ...
      <div>
        {actionType === "AddFunds" ? (
          <div className="ProfileFunds">
            ...
            <form>
              <div class="mb-3">
                <label htmlFor="amountInput" class="form-label">
              ...
              <div class="mb-3">
                <label htmlFor="selectInput" class="form-label">
